    ...
    if (swap) {
        ...
        for (i = 0; i < artistLen; i++) {
            ...
            if (cJSON_GetArraySize(artist) > 0) {
                ...
                mArtist +=
                        std::string(cJSON_GetStringValue(cJSON_GetArrayItem(artist, 0)));
            }
        }
    }
...
void NeteaseCrypt::FixMetadata() {
    ...
    // tag->setComment(TagLib::String("Create by taurusxin/ncmdump.",
    // TagLib::String::UTF8));
...
NeteaseCrypt::NeteaseCrypt(std::string const &path) {
    ...
    if (n <= 0) {
        std::cout << BOLDYELLOW << "[Warn] " << RESET << "'" << path
                  << "' missing metadata infomation can't fix some infomation!"
                  << std::endl;
    ...
    if (n > 0) {
        ...
        std::cout << BOLDYELLOW << "[Warn] " << RESET << "'" << path
                  << "' missing album can't fix album image!" << std::endl;
    }
